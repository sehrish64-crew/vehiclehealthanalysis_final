...
export default function AdminDashboard() {
      ...
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((star) => (
          <Star
            key={star}
            className={`w-5 h-5 ${
              star <= rating ? 'fill-[#0f4c81] text-[#0f4c81]' : 'text-gray-300'
            }`}
          />
        ))}
      </div>
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-[#9EB8D3] text-[#0d3a64] border border-[#9EB8D3]">Pending</span>
      case 'processing':
                      ...
                      <div className="flex gap-1 md:gap-2 flex-wrap">
                        {pendingTab ? (
                          <>
                            ...
                            <Button onClick={() => handleReject(review.id)} variant="outline" className="text-[#0f4c81] border-[#9EB8D3] hover:bg-[#D0E3F1]" size="sm"><span className="hidden md:inline">Reject</span></Button>
                          </>
                        ) : (
