    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
      <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden flex flex-col animate-in scale-in duration-300">
        ...
        <div className="relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-r from-[#0f4c81] via-[#2d5ca1] to-[#0f4c81] opacity-95"></div>
          <div className="absolute inset-0 opacity-15">
            <div className="absolute top-0 right-0 w-96 h-96 bg-[#0f4c81] rounded-full blur-3xl"></div>
            <div className="absolute bottom-0 left-0 w-96 h-96 bg-[#2d5ca1] rounded-full blur-3xl"></div>
          </div>
        ...
        <div className="p-6 border-b border-[#0f4c81]/20 bg-gradient-to-b from-[#0f4c81]/5 to-background">
          <div className="relative group">
            <Search className="absolute left-4 top-3.5 w-5 h-5 text-[#0f4c81]/60 group-focus-within:text-[#0f4c81] transition duration-200" />
            <input
              type="text"
              placeholder="Search countries, codes, or currencies..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-12 pr-6 py-3 border-2 border-[#0f4c81]/30 rounded-xl focus:outline-none focus:border-[#0f4c81] focus:ring-4 focus:ring-[#0f4c81]/20 transition-all duration-200 text-foreground placeholder-muted-foreground bg-white"
              autoFocus
            />
          </div>
        </div>
                ...
                <button
                  ...
                  className={`group p-4 rounded-xl transition-all duration-200 text-left border-2 ${
                    selectedCountry.code === country.code
                      ? 'bg-gradient-to-br from-[#0f4c81]/15 to-[#2d5ca1]/10 text-[#0f4c81] border-[#0f4c81] shadow-lg shadow-[#0f4c81]/30 scale-105'
                      : 'bg-gradient-to-br from-gray-50 to-gray-100 text-foreground border-gray-200 hover:border-[#0f4c81]/40 hover:shadow-md hover:scale-102 hover:bg-gradient-to-br hover:from-[#0f4c81]/5 hover:to-[#2d5ca1]/5'
                  }`}
                >
                  <div className="flex items-start justify-between mb-2">
                    <div className="font-bold text-lg">{country.name}</div>
                    {selectedCountry.code === country.code && (
                      <span className="inline-flex items-center justify-center w-6 h-6 bg-[#0f4c81]/20 rounded-full">
                        <Zap className="w-4 h-4 text-[#0f4c81]" />
                      </span>
                    )}
                  </div>
                  <div className={`text-sm font-semibold mb-1 ${selectedCountry.code === country.code ? 'text-[#0f4c81]' : 'text-[#0f4c81]'}`}>
                    {country.currency}
                  </div>
                  <div className={`text-xs ${selectedCountry.code === country.code ? 'text-[#0f4c81]/70' : 'text-muted-foreground'}`}>
                    One Time Payment
                  </div>
                  <div className={`text-xs mt-2 font-semibold ${selectedCountry.code === country.code ? 'text-[#0f4c81]' : 'text-foreground'}`}>
                    {country.code}
                  </div>
                </button>
              ))}
        ...
        {/* Enhanced Footer */}
        <div className="border-t border-[#0f4c81]/20 p-6 bg-gradient-to-r from-[#0f4c81]/5 via-background to-[#0f4c81]/5 flex justify-between items-center">
          <div>
            <p className="text-xs text-[#0f4c81]/70 uppercase tracking-wide font-semibold mb-1">Currently Selected</p>
            <p className="text-lg font-bold text-[#0f4c81]">{selectedCountry.name} ({selectedCountry.code})</p>
          </div>
          <button
            onClick={() => setIsOpen(false)}
            className="px-8 py-3 bg-gradient-to-r from-[#0f4c81] to-[#0f4c81] text-white rounded-xl hover:shadow-lg hover:shadow-[#0f4c81]/40 hover:scale-105 transition-all duration-200 font-semibold flex items-center gap-2 group"
          >
            <span>Continue</span>
