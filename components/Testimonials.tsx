...
export default function Testimonials() {
  ...
  const renderStars = (rating: number) => (
    <div className="flex gap-1 mb-4">
      {[1, 2, 3, 4, 5].map((star) => (
        <Star
          key={star}
          className={`w-5 h-5 ${star <= rating ? 'fill-[#0f4c81] text-[#0f4c81]' : 'text-gray-300'
            }`}
        />
      ))}
    </div>
  ...
  return (
    <section
      ref={sectionRef}
      className="relative py-20 md:py-28 bg-gradient-to-br from-[#0f4c81]/10 via-white to-[#0f4c81]/20 overflow-hidden"
    >
      <div className="absolute top-10 left-10 w-72 h-72 bg-[#0f4c81] rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob"></div>
      <div className="absolute top-10 right-10 w-72 h-72 bg-[#0f4c81] rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-2000"></div>
      <div className="absolute bottom-10 left-1/2 w-72 h-72 bg-[#0f4c81] rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-4000"></div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="max-w-xl px-3">
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex -space-x-3">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="w-12 h-12 rounded-full bg-[#0f4c81] border-4 border-white flex items-center justify-center">
        ...
        {isLoading ? (
          <div className="text-center py-12">
            <div className="w-12 h-12 border-4 border-[#0f4c81] border-t-transparent rounded-full animate-spin mx-auto"></div>
          </div>
        ) : (
          <div className="grid md:grid-cols-3 gap-8">
            ...
            {visibleReviews.map((review) => (
              <div key={review.id} className="bg-white p-8 rounded-2xl shadow-lg hover:-translate-y-2 transition">
                ...
                <Quote className="w-10 h-10 text-[#0f4c81]" />
                ...
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-[#0f4c81] flex items-center justify-center">
