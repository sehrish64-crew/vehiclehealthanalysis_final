...
export async function sendCustomerPaymentConfirmation(data: PaymentConfirmationEmailData) {
    ...
    <html>
      <head>
        <style>
          ...
          .details { background: white; padding: 15px; margin: 15px 0; border-left: 4px solid #0f4c81; }
          .detail-row { margin: 8px 0; }
          .label { font-weight: bold; color: #0f4c81; }
          .button { background: #0f4c81; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block; margin: 15px 0; }
        </style>
      </head>
      <body>
        <div class="container">
          ...
          <div class="footer">
            <p>&copy; 2024 Vehicle Health Analysis. All rights reserved.</p>
  ...
  try {
    await transporter.sendMail({
      ...
      subject: `Payment Confirmed - Vehicle Health Analysis Order #${data.orderId}`,
      html: htmlContent,
    })
